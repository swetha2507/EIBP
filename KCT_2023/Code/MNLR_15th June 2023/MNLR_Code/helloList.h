...
void findParent_T1_from_T3(char* myTierAdd, char* parentTierAdd) {
	...
	while (fNode != NULL) {
		...
		if (strlen(myTierAdd) > strlen(temp)) {
			if (enableLogScreen)
				printf("findParntLongst Inside first if Tier 3 Address = %s  and Tier 1 Address = %s", myTierAdd, temp);

			if (checkIfSubstring(myTierAdd, temp)) {
				if (enableLogScreen)
					printf("findParntLongst Inside second if Tier 3 Address = %s and Tier 1 Address = %s", myTierAdd, temp);
