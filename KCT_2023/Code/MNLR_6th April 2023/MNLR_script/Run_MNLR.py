...
def trigger_MNLR(info_dict, user, pswd, keyLocation, cmd_file, remoteDst):
    ...
    for i in range(0, len(nodes)):
        ...
        if  not  nodes[i][:-1] == 'ipnode': #ipnode or host or IPN
            ...
            stdin, stdout, stderr = ssh[i].exec_command('sudo rm ' + remoteDst + '/MNLR_Code/mnlrlogs', timeout=30)
            ...
            #stdin, stdout, stderr = ssh[i].exec_command('cd '+ remoteDst +'/MNLR_Code/;gcc -o MNLR *.c -lm', timeout=30)
            #time.sleep(2)
            ...
            x ='cd ' + remoteDst + '/MNLR_Code/; sudo screen -dmS ' + str(nodes[i]) + ' -L -Logfile mnlrlogs ' + str(Mcmd_dict[nodes[i]])
            print(x)
...
if __name__ == "__main__":
    ...
    trigger_MNLR(eval(open(info_file).read()), uname, password, keyLocation, cmd_file, remoteDst)
    fileop.close()
