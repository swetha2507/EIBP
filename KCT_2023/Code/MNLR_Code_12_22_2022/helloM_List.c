	...
	// printf("\n argv[0]= %s",argv[0] );
		...
		if (argc > 1) { //if the number of arguments is greater than 1, we will go forward
			...
			while (argc > 0) { //the loop will continue until the number of arguments is greater than 1. 
				...
				if (argc > 0) {
					opt = argv[0];
					if (!(strcmp(opt, "-T") == 0) && !(strcmp(opt, "-N") == 0))  //if -N or -T does not come after label address of node, then it is an error. 
					{
						argc--;
						argv++;
						totalIPError++;
		...
		//tier 3 will be connected to ip node - KCT 22/12/2022
		if(myTierValue==3) {
			int z = 0;
