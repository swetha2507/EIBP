...
def trigger_MNLR(info_dict, user, pswd, keyLocation, cmd_file, remoteDst):
    ...
    # iterate over the list of commands and modify as needed
    i=0
    for i,line in enumerate(cmd_lst):
        node, command = line.strip().split(',')
        ...
        if '-N 0' in command:
            ...
            stdin, stdout, stderr = ssh[i].exec_command(f'ifconfig {interface} | grep "inet " | head -n 1 | awk \'{{print $2}}\'')
            network_address = stdout.read().strip()
            netmask = '24' # assuming netmask is always /24
            new_command = f'{command.split("-N 0")[0]}-N 0 {network_address} {netmask} {interface}'
            cmd_lst[i] = f'{node},{new_command}'
            cmd_lst[i] = cmd_lst[i].replace("b'", "").replace("'", "")
    ssh[i].close()
